...
본 프로젝트는 딥러닝 없이 순수 영상처리 기법만으로 전방 위험 객체 탐지 시스템을 구현하였습니다.

| 항목 | 내용 |
|------|------|
| 객체 탐지 | Canny Edge + Contour / Otsu + Connected Component 2단계 검출 |
| 위험도 분류 | 크기·중앙 거리·시인성 3요소 기반 SAFE / CAUTION / DANGER 등급화 |
| 환경 적응 | 시인성 점수로 야간/역광 상황 자동 보정 |
| 강의 연계 | Lec 3~7 핵심 기법을 단일 파이프라인으로 통합 |

### 부족한 부분 (현재 한계)

**1. 거리 추정 부정확**
단안 카메라의 근본적 한계로, 실제 거리를 측정하지 못하고 객체 면적으로 추정합니다.
멀리 있는 대형 트럭이 가까운 소형차보다 DANGER로 분류되는 오분류가 발생할 수 있습니다.

**2. 클래스 구분 불가**
차량인지 보행자인지 오토바이인지 구분하지 못합니다.
객체 유형에 따라 위험도 기준이 달라져야 하지만 현재는 동일 기준 적용합니다.

**3. 조명/날씨 변화에 취약**
히스토그램 평탄화로 일부 보정하지만, 폭우·안개·야간처럼 엣지 자체가 사라지는 환경에서는 검출 실패율이 높습니다.

**4. 겹침 처리 불가**
두 차량이 겹쳐 있으면 하나의 큰 덩어리로 인식합니다.

**5. 속도/방향 정보 없음**
현재 프레임만 분석하므로 물체가 접근 중인지 멀어지는 중인지 알 수 없습니다.

**6. Harris/SIFT가 위험도에 미반영**
특징점 수를 HUD에 표시하지만 실제 위험도 점수 계산에는 사용되지 않습니다.

---

### 추후 연구 방향

기존 입력 파이프라인(전처리 → 객체 감지 → 특징점 추출 → 위험도 평가)을 유지하면서 각 단계를 딥러닝으로 강화하는 방향입니다.
...
단안 카메라 입력
    │
    ▼
전처리 (Gaussian, Canny, Histogram)
    │
    ▼
객체 감지 (Contour / Connected Component)
    │
    ▼
특징점 추출 (Harris, SIFT)
    │
...
**[1단계] 객체 감지 → CNN 기반 검출기로 교체**

Contour/Connected Component 대신 **YOLO** 또는 **SSD** 로 교체합니다.
파이프라인 구조는 그대로 유지하고 `detection.py` 의 검출 함수만 교체합니다.

- 클래스 구분 가능 (차량 / 보행자 / 오토바이 / 신호등)
- 겹침 객체 분리 가능
- 조명·날씨 변화에 강인
...
전처리 결과 이미지
    │
    ▼
YOLO 검출 → (class, bbox, confidence) 반환
    │
    ▼
기존 위험도 평가 단계로 전달 (파이프라인 유지)
```

**[2단계] 위험도 평가 → 실제 거리 기반으로 개선**

크기로 거리를 추정하는 한계를 극복하기 위해 **Monocular Depth Estimation** 을 추가합니다.
MiDaS 또는 Depth Anything 모델로 각 픽셀의 상대 깊이를 추정하여 객체까지의 거리를 `risk.py` 에 반영합니다.
...
전처리 결과 이미지
    ...
    ├──→ 기존 객체 감지
    │
    └──→ MiDaS 깊이 추정 → 객체 영역 평균 깊이 계산
                               │
                               ▼
                    크기 점수 대신 실제 거리 점수 사용
```

**[3단계] 특징점 추출 → 객체 추적으로 확장**

Harris/SIFT 특징점을 단순 카운트에서 그치지 않고 **프레임 간 매칭** 에 활용합니다.
SIFT 디스크립터 매칭으로 동일 객체를 추적하면 속도와 접근 방향을 계산할 수 있습니다.

```
프레임 N 특징점 ──┐
                  ├──→ SIFT 매칭 → 이동 벡터 계산 → 접근/이탈 판단
프레임 N+1 특징점 ┘
```
...
전처리 (유지)
    ...
    │
    ▼
위험도 평가 (거리 + 속도 + 클래스 + 시인성) → SAFE / CAUTION / DANGER
```
