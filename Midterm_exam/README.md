...
| 개선 항목 | 방법 |
|----------|------|
| 속도 추정 추가 | Optical Flow(Lucas-Kanade)로 프레임 간 객체 이동 벡터 계산 |
| 겹침 객체 분리 | Watershed Segmentation 적용 |
| Harris/SIFT 위험도 반영 | 특징점 수를 위험도 보조 점수로 편입 (구조 복잡도 = 차량 가능성) |
| 야간 성능 개선 | CLAHE(제한 대비 히스토그램 평탄화)로 더 세밀한 보정 |
| ROI 설정 | 도로 영역만 분석하도록 Perspective Transform으로 Bird's Eye View 변환 |

**중장기 개선 — 딥러닝 도입**

| 개선 항목 | 방법 |
|----------|------|
| 정확한 객체 탐지 | YOLO, SSD 등 딥러닝 검출기로 교체 → 클래스 구분 및 정확도 향상 |
| 단안 깊이 추정 | Monocular Depth Estimation (MiDaS, Depth Anything) 으로 실제 거리 추정 |
| 다중 프레임 추적 | DeepSORT 등 트래커 추가 → 객체별 속도/가속도 기반 위험도 예측 |
| 차선 인식 통합 | 차선 검출을 결합해 차선 이탈 여부도 위험도에 반영 |

**궁극적 목표**

```
현재:  단안 카메라 → 영상처리 → 크기/위치 기반 위험도
목표:  단안 카메라 → 딥러닝 탐지 + 깊이 추정 + 객체 추적
                  → 실제 거리 + 속도 + 클래스 기반 위험도 예측
```
